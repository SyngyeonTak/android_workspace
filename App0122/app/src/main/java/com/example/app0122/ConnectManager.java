...
자바도 html문서처럼, 웹서버와 http 통신이 가능하다...
*/
...
public class ConnectManager extends Thread{
    ...
    URL url;
    HttpURLConnection con;//http통신을 위한 객체(헤더 + 바디를 구성하여 서버와 데이터를 주고받는 방식이다.
    //stateless한 통신)
    ...
    public ConnectManager(MainActivity mainActivity, String requestUrl, String data){//이 객체를 생성하는 자는 주소와 제이슨 데이터를 넘겨야 한다.
    ...
    public int requestByGet(){//Get방식으로 요청을 시도하는 메서드
        ...
        try{
            ...
            String data = null;
            while(true){
                data = buffr.readLine();//한줄 읽어 들인다.
                if(data == null) break; //읽어들일 데이터가 없다면 무한 루프 종료
                sb.append(data);//읽어들인 문자열을 누적시키자
            }

            Log.d(TAG, "서버가 보낸 응답데이터: "+sb.toString());

            code = con.getResponseCode();//서버로 부터 받은 응답코드 반환(이 시점에 이미 서버에 요청을 완료후 응답도 받은 상태이다.)
            Log.d(TAG, "서버로 부터 받은 응답 코드: "+code);

            //이 시점이 바로 통신이 완료된 시점이므로, 여기서 무언가를 해야한다.
            //mainActivity.printData(sb.toString());//사용자 정의 쓰레드는 디자인 접근 불가
            //해결책? 디자인을 갱신해달라고, 요청하면 된다. 이때 요청을 받는 전담 객체를 핸들러라 한다.
            //핸들러에게 동작을 요청!!!
            //handler의 handlerMessage()를 호출하게 됨
            Message message = new Message();//empty message 객체 생성
    ...
    public int requestByPost(){
        ...
        try{
            url=new URL(requestUrl);//요청 주소
            con=(HttpURLConnection)url.openConnection();
            //데이터형식을 헤더에 첨가해줘야, 서버측에서 제이슨데이터가 전송되어 온 것임을 안다.이게 바로 HTTP프로토콜간의 약속이다.
            con.setRequestProperty("Content-Type","application/json;charset=utf-8");
            con.setRequestMethod("POST");
            con.setDoOutput(true);//서버에 데이터를 출력하기 위해 필요한 옵션!!

            //요청을 떠나기 전에, 준비할게 있다면 여기서 준비하자!!, JSON문자열을 준비하자
            //JSON객체 자체가 아닌 문자열로 준비하는 이유는? "{}" 문자열로 serial되어야 됨
