...
public class ConnectManager extends Thread{
    ...
    public ConnectManager(String requestUrl, String data){//이 객체를 생성하는 자는 주소와 제이슨 데이터를 넘겨야 한다.
    ...
    public void requestByGet(){//Get방식으로 요청을 시도하는 메서드
        try{
            url=new URL("http://192.168.0.8:7777/rest/member");//요청 주소
            ...
            System.out.println("서버가 보낸 응답데이터: "+sb.toString());

            int code = con.getResponseCode();//서버로 부터 받은 응답코드 반환(이 시점에 이미 서버에 요청을 완료후 응답도 받은 상태이다.)
            System.out.println("서버로 부터 받은 응답 코드: "+code);
